...
export default function CreateObject() {
  ...
  useEffect(() => {
    if (typeof lom.general.keyword === 'string') {
      setLom((v) => ({
        ...v,
        general: { ...v.general, keyword: v.general.keyword.split(',').map((s) => s.trim()).filter(Boolean) },
      }));
    }
  }, []);
  ...
  function buildMetadata() {
    ...
    if (advanced) {
      ...
      const lifecycle = {
        ...
        contribute:
          lom.lifecycle.contributeRole || lom.lifecycle.contributeEntity || lom.lifecycle.contributeDate
            ? [
                {
  ...
  return (
    <div className={css.page}>
      ...
      {notice && <div className={`${css.banner} ${css.bannerSuccess}`} aria-live="polite">{notice}</div>}
              ...
              <div className={css.accordionWrapper}>
                ...
                <div className={css.fieldsGrid}>
                  ...
                  <div className={css.fieldHalf}>
                    ...
                    <select value={lom.general.language} onChange={onLom('general', 'language')} className={css.select}>
                  ...
                  <div className={css.fieldThird}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLom((prev) => ({
                          ...prev,
                          lifecycle: { ...prev.lifecycle, versionNum: Math.max(1, parseInt(e.target.value || '1', 10)) },
                        }))
                      }
                      className={css.input}
                    />
                  ...
                  <div className={css.fieldsGrid}>
                    <div className={css.fieldHalf}>
                      <label className={css.label}>Custo</label>
                        <select value={lom.rights.cost} onChange={onLom('rights', 'cost')} className={css.select}>
                          {ENUMS.cost.map(o => (
      ...
      <InstallExtensionModal open={installOpen} onClose={() => setInstallOpen(false)} payload={installPayload} error={installError} publicUrl={null} />
    </div>
  );
}
